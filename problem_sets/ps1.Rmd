...
> A quick exploration of the data in a spreadsheet and using summary(d) quickly shows that there are supposed to be 3 experimental conditions with 30 participants in each. However, there are some data in the input and answers which aren't formatted correctly.
```{r}
summary(d)
```

> This is confirmed by checking levels for the Answer columns above, e.g.: 
```{r}
levels(d$Answer.dog_cost)
levels(d$Answer.plasma_cost)
levels(d$Answer.sushi_cost)
```


> To fix that (and convert the levels to numeric values):
```{r}
d$Answer.dog_cost <- revalue(d$Answer.dog_cost, c("five hundred"=500, "2,000"=2000))
...
d$Answer.dog_cost <- as.numeric(as.character(d$Answer.dog_cost))
d$Answer.sushi_cost <- as.numeric(as.character(d$Answer.sushi_cost))
d$Answer.plasma_cost <- as.numeric(as.character(d$Answer.plasma_cost))
```

> The input prices are also read in as strings because of the commas.
```{r}
d$Input.price1 <- as.numeric(gsub(",","", d$Input.price1))
d$Input.price2 <- as.numeric(gsub(",","", d$Input.price2))
d$Input.price3 <- as.numeric(gsub(",","", d$Input.price3))
```

> Additionally, the summary clearly shows that 2 participants completed the task more than once. To fix this, we can keep only the first result for each participant.
```{r}
d[which(d$WorkerId == "A1VYX5VKZ0CTWU"),]
d <- subset(d, AssignmentId != "2IBHSTLB3L0FR1IX01AEQREVGHDIU3" & AssignmentId != "22LLB3L0FBJ9Y6KYQ9IE2E5RR3PXLP")

d[which(d$WorkerId == "A5IUO2T5MZQUZ"),]
d <- subset(d, AssignmentId != "2RSJPPSI2KYE5314JVBLT6WJ500QKL")
```

> We could also drop AssignmentId=2C2ESCWY2AOOIYV1Q6FONQSCOG111N as well, since it has an incomplete answer for sushi_cost and was completed in only 18s.
```{r}
d <- subset(d, AssignmentId != "2C2ESCWY2AOOIYV1Q6FONQSCOG111N")
```


> To confirm that the fixes were done correctly:
```{r}
summary(d)
```

> which shows that the Answers are all numeric and there aren't duplicate WorkerId's. Each input condition still has equal numbers of participants as well.
...
d <- subset(d, WorkerId != "A3S99S9NJ6X08J") # drop row with NA sushi cost
```
...
```{r}
ggplot(d.tidy, aes(x=cost, fill=item)) +
    geom_histogram(position=position_dodge())
```

> The data is difficult to interpret because of the different scales. If we allow for different scales with each item, we can interpret the sushi data better. However, it looks like there might not be any correlations between conditions and outcomes.

```{r}
ggplot(data=d.tidy, aes(x=cost, fill=condition)) +
...
```{r}
grouped <- group_by(d.tidy, item)
summarise(grouped, mean=mean(cost))
```
...
```{r}
pcts <- d.tidy %>% 
  mutate(pct_change = (cost - anchor) * 100 /anchor)
pcts %>% 
...
z.scores <- d.tidy %>% 
  group_by(item) %>%
  mutate(mu = mean(cost), sd = sd(cost)) %>%
  ungroup() %>%
  mutate(z = (cost - mu) / sd)
  
z.scores %>% group_by(item, condition, add=FALSE) %>%
 summarize("mean z" = mean(z))
```
...
```{r}
ggplot(pcts, aes(x = item, y = pct_change, fill =condition )) +
