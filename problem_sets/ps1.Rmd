...
d.tidy <- d %>% 
  select(WorkerId, Input.condition, 
         starts_with("Answer"), 
         starts_with("Input"))
#  select(-starts_with("HIT"), -Title, -Description, -Keywords, -Reward,
#         -CreationTime, -MaxAssignments, -RequesterAnnotation,
#         -RejectionTime, -RequesterFeedback, -LifetimeApprovalRate,         
#         -Last30DaysApprovalRate, -Last7DaysApprovalRate,
#         -AssignmentStatus, -ApprovalTime)
...
```{r rename}
d.tidy <- d.tidy %>% 
#  rename(Answer.DogCost = Answer.dog_cost) %>%
#  rename(Answer.PlasmaCost = Answer.plasma_cost) %>%
#  rename(Answer.SushiCost = Answer.sushi_cost)
...
> The values for sushi are much lower than that for dogs and plasma.

```{r}
qplot(cost, facets = ~ name, data=d.tidy)
```
